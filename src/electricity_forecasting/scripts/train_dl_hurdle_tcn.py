"""Train DL Hurdle TCN (volume-only) and save the same artifacts as the notebook.

Design goals:
- Same train/test split: train=2021-2023, test=2024
- Same WIN/HORIZON windowing
- Same window_index storage format: (ID_code, start_pos)
- Validation windows are sampled from the last 10% of the train period (time-based)
- Threshold selection is done on validation only (sweep), test is evaluated once

Artifacts (paths relative to PROJECT_DIR):
- results/models/dl_hurdle_tcn_best.pt
- results/metrics/dl_hurdle_tcn.json
- results/metrics/metrics_table_dl_hurdle_tcn_vs_baseline.(csv/json)
- results/plots/models/dl_hurdle_tcn/baseline_vs_model_full_active.png
- results/plots/eval/curves/dl_hurdle_tcn_threshold_sweep_volume_full.png
- results/plots/eval/tables/metrics_table_dl_hurdle_tcn_vs_baseline.png
"""
...
from electricity_forecasting.models.dl_hurdle_tcn import HurdleTCN
...
def parse_args():
    ...
    p.add_argument("--ckpt_path", type=str, default="", help="Optional checkpoint path. Default: results/models/dl_hurdle_tcn_best.pt")
    ...
    p.add_argument("--pos_weight", type=float, default=-1.0, help="If <0, auto-compute from train windows (sampled).")
...
def main():
    ...
    (plots_dir / "models/dl_hurdle_tcn").mkdir(parents=True, exist_ok=True)
    ...
    if ckpt is not None and isinstance(ckpt, dict) and "feature_cols" in ckpt:
        ...
        # Build one ID grid and infer feature columns.
    ...
    # 4) Datasets / Loaders (streaming via window_index)
    ...
    # 5) Model
    channels = [int(x) for x in str(args.channels).split(",") if str(x).strip()]
    if ckpt is not None and isinstance(ckpt, dict) and "channels" in ckpt:
        channels = [int(x) for x in ckpt["channels"]]
        print("Channels (from ckpt):", channels)
    model = HurdleTCN(
    ...
    if not args.eval_only:
        ...
        # eval_only: pos_weight does not affect evaluation; keep the flag value for metadata if provided.
        if float(args.pos_weight) > 0:
            pos_weight = torch.tensor(float(args.pos_weight), dtype=torch.float32)
    ...
    if args.eval_only:
        ...
        ckpt_path = results_dir / "models" / "dl_hurdle_tcn_best.pt"
        torch.save(
            {
    ...
    sweep_csv = results_dir / "metrics" / "threshold_sweep_dl_hurdle_tcn.csv"
    sweep_df.to_csv(sweep_csv, index=False)

    # Plot sweep curve in the same style as ML script
    ...
    sweep_png = plots_dir / "eval/curves" / "dl_hurdle_tcn_threshold_sweep_volume_full.png"
    plot_threshold_sweep(sweep_plot_df, out_png=str(sweep_png), title="DL Hurdle TCN: threshold sweep (Volume)")
    ...
    bar_path = plots_dir / "models/dl_hurdle_tcn" / "baseline_vs_model_full_active.png"
    plot_and_save_baseline_vs_model_bar(
        metrics_bar=metrics_bar,
        out_png=str(bar_path),
        title="DL Hurdle TCN (Volume only) vs Baseline",
        dpi=160,
    )
    ...
    record = build_metrics_record(
        ...
        run_tag="dl_hurdle_tcn_window_based",
        constants={
            "WIN": int(WIN),
            "HORIZON": int(HORIZON),
        ...
        extra={
            ...
            "device": str(device),
            "arch": "Hurdle TCN (BCE + masked Huber on log1p(volume))",
            "channels": channels,
            "dropout": float(args.dropout),
            "lr": float(args.lr),
            "weight_decay": float(args.weight_decay),
